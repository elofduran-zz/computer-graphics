...
class Sphere(Sphere):
    ...
    def __init__(self):
        self.rings = 10
        self.sectors = 10
        self.R = 1. / float(self.rings - 1)
        self.S = 1. / float(self.sectors - 1)
        self.t = 0
        self.v = [[]]
        self.radius = 0.5
        self.type = "CYLINDER"
    ...
    def draw(self):
        for r in range(self.rings):
            for s in range(self.sectors):
                z = math.cos(2 * math.pi * s * self.S)
                x = math.sin(2 * math.pi * s * self.S)
                y = r/4

                y = math.sin(-math.pi / 2 + math.pi * r * self.R)
                x = math.cos(2 * math.pi * s * self.S) * math.sin(math.pi * r * self.R)
                z = math.sin(2 * math.pi* s * self.S) * math.sin(math.pi * r * self.R)

                self.v[self.t][0] = x * self.radius
                self.v[self.t][1] = y * self.radius
                self.v[self.t][2] = z * self.radius
                self.t += 1
